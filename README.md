...
- Go 1.26.x
- make
- `golangci-lint`（lint を使う場合）

プロバイダ別の要件:

- `copilot`: `GITHUB_TOKEN` 環境変数が必要（CLI は不要）
- `ollama`: `ollama serve` が稼働しているか、ローカル Ollama サーバーに接続できること。推奨モデルは Makefile の `OLLAMA_MODEL` で取得できます。
...
# サンプル実行（要件文字列を渡す）
go run ./cmd/orchestrator "文字列を逆順にする関数を追加してください"
...
```bash
go run ./cmd/orchestrator "HTTP ハンドラにバリデーションを追加して"
```
...
```bash
go run ./cmd/orchestrator https://github.com/owner/repo/issues/42
```
...
|                                   コマンド | 説明                                                   |
| -----------------------------------------: | ------------------------------------------------------ |
|       `go run ./cmd/orchestrator "<要件>"` | 要件文字列で実行                                       |
| `go run ./cmd/orchestrator --issue <番号>` | GitHub Issue 番号で実行                                |
|    `go run ./cmd/orchestrator <issue-url>` | Issue URL を直接指定                                   |
...
make ollama-pull  # Ollama 推奨モデルを pull
```

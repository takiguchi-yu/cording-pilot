// Package llm は大規模言語モデル（LLM）と連携するための Strategy インターフェースを定義します。
package llm

import (
	"context"
	"errors"
)

// ErrJSONParse は LLM が返したレスポンスを JSON としてデコードできなかった場合のエラーです。
// Fix Loop でのフィードバック検出に使用されます。
var ErrJSONParse = errors.New("JSONの形式が不正です")

// Client は LLM 通信の Strategy インターフェースです。
// 実装はゴルーチン安全である必要があります。
type Client interface {
	// Generate はプロンプトを LLM に送信し、生成されたテキストを返します。
	Generate(ctx context.Context, prompt string) (string, error)
	// GenerateStructured はプロンプトを LLM に送信し、JSON レスポンスを target にデコードします。
	// JSON デコードに失敗した場合は ErrJSONParse をラップしたエラーを返します。
	// target は JSON アンマーシャル可能なポインタを渡してください。
	GenerateStructured(ctx context.Context, prompt string, target interface{}) error
}

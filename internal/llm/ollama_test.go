...
func TestOllamaClient_GenerateStructured_壊れたJSONを1回修復してデコードする(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		content := "{\"files\":[{\"path\":\"a.txt\",\"content\":\"x\"}],\"broken\":"
		if callCount >= 2 {
			content = "{\"files\":[{\"path\":\"a.txt\",\"content\":\"x\"}]}"
		}
	...
	if callCount < 2 {
		t.Fatalf("callCount=%d; want >=2 (initial + repair)", callCount)
	}
	if len(out.Files) != 1 || out.Files[0].Path != "a.txt" {
		t.Fatalf("unexpected decoded output: %+v", out)
	}

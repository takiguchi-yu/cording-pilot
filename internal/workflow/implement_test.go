...
func TestImplementState_Execute_PhaseA_JSON解析エラー時にテスト生成を再試行する(t *testing.T) {
	t.Parallel()
	...
	if testGenCallCount != 2 {
		t.Errorf("testGenCallCount=%d; want 2", testGenCallCount)
	}
}
...
func TestImplementState_Execute_同一条件では実装生成キャッシュを再利用する(t *testing.T) {
	...
	if implCallCount != 2 {
		t.Errorf("implCallCount=%d; want 2 (初回条件と失敗フィードバック条件の2通り)", implCallCount)
	}
}
...
func TestImplementState_Execute_PhaseA_全試行がGreenでも警告して継続する(t *testing.T) {
	t.Parallel()
	...
	// maxTestRedRetries 回すべて Green を返し、継続後の Fix Loop で Green。
	exec := &stubExecutor{
		responses: []execResponse{
			{output: "ok", success: true},
			{output: "ok", success: true},
